...
pub struct ObCrc64Sse42 {
}

impl ObCrc64Sse42 {
    pub fn crc64_sse42_manually(mut crc: i64, data: &[u8]) -> i64 {
        for &byte in data {
            crc = CRC32_TAB[((crc ^ byte as i64) & 0xff) as usize] ^ (crc >> 8);
        }
        crc
    }

    pub fn fast_crc64_sse42_manually(crc: i64, data: &[u8]) -> i64 {
        let mut local_crc: i64 = crc;
        let mut len: i64 = data.len() as i64;
        let mut offset: usize = 0;

        while len > 7 {
            let c0 = data[offset] ^ local_crc as u8;
            let c1 = data[offset + 1] ^ (local_crc >> 8) as u8;
            let c2 = data[offset + 2] ^ (local_crc >> 16) as u8;
            let c3 = data[offset + 3] ^ (local_crc >> 24) as u8;
            local_crc = (T8_7[c0 as usize] ^ T8_6[c1 as usize]) ^ (T8_5[c2 as usize] ^ T8_4[c3 as usize]);

            let c3 = T8_3[data[offset + 4] as usize];
            let c2 = T8_2[data[offset + 5] as usize];
            let c1 = T8_1[data[offset + 6] as usize];
            let c0 = T8_0[data[offset + 7] as usize];
            local_crc ^= (c3 ^ c2) ^ (c1 ^ c0);
    ...
    pub fn fast_crc64_sse42(crc: i64, data: &[u8]) -> i64 {
        if !is_x86_feature_detected!("sse4.2") {
            ObCrc64Sse42::fast_crc64_sse42_manually(crc, data)
        } else {
            let mut local_crc: i64 = crc;
            let mut len: i64 = data.len() as i64;
            let mut offset: usize = 0;
            ...
            while len > 7 {
                local_crc = unsafe { _mm_crc32_u64(local_crc as u64, LittleEndian::read_u64(&data[offset..offset + 8])) as i64 };
                len -= 8;
                offset += 8;
            }
...
#[cfg(test)]
mod test {
    use crate::rpc::util::checksum::ob_crc64::ObCrc64Sse42;

    #[test]
    fn crc64_sse42_manually_test() {
        let s = String::from("StringNeedToBeCalculatedCheckSumAndTest,Make String longer longer longer longer longer to test");
        let n = ObCrc64Sse42::crc64_sse42_manually(0, &s.into_bytes());
        assert_eq!(n, 1566783161);
    }

    #[test]
    fn fast_crc64_sse42_manually_test() {
        let s = String::from("StringNeedToBeCalculatedCheckSumAndTest,Make String longer longer longer longer longer to test");
        let n = ObCrc64Sse42::fast_crc64_sse42_manually(0, &s.into_bytes());
        assert_eq!(n, 1566783161);
    }

    #[test]
    fn both_crc64_sse42_manually_test() {
        let s = String::from("String Need To Be Calculated CheckSum And Test,Make String longer longer longer longer longer to test");
        assert_eq!(ObCrc64Sse42::fast_crc64_sse42_manually(0x05010927, &s.to_owned().into_bytes()), ObCrc64Sse42::crc64_sse42_manually(0x05010927, &s.into_bytes()));
    }

    #[test]
    fn fast_crc64_sse42_test() {
        let s = String::from("StringNeedToBeCalculatedCheckSumAndTest,Make String longer longer longer longer longer to test");
        let n = ObCrc64Sse42::fast_crc64_sse42(0, &s.into_bytes());
        assert_eq!(n, 1566783161);
    }
}
