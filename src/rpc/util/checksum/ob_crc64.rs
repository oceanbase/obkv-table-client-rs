...
impl ObCrc64Sse42 {
    ...
    pub fn fast_crc64_sse42_manually(crc: i64, data: &[u8]) -> i64 {
        ...
        while len > 7 {
            let c0 = data[offset as usize] ^ local_crc as u8;
            let c1 = data[(offset + 1) as usize] ^ (local_crc >> 8) as u8;
            let c2 = data[(offset + 2) as usize] ^ (local_crc >> 16) as u8;
            let c3 = data[(offset + 3) as usize] ^ (local_crc >> 24) as u8;
            local_crc = (T8_7[c0 as usize &0xff] ^ T8_6[c1 as usize & 0xff]) ^ (T8_5[c2 as usize & 0xff] ^ T8_4[c3 as usize & 0xff]);

            let c3 = T8_3[data[(offset + 4) as usize] as usize & 0xff];
            let c2 = T8_2[data[(offset + 5) as usize] as usize & 0xff];
            let c1 = T8_1[data[(offset + 6) as usize] as usize & 0xff];
            let c0 = T8_0[data[(offset + 7) as usize] as usize & 0xff];
